...
class TrainUtils:
    ...
    def save_model(self):
        ...
        print('Attempting to save models to', save_folder)
        for model_name, model in self.models.items():
            print(f"Saving {model_name} weights...")
        ...
        if not self.models:
            save_path = os.path.join(save_folder, f"segformer.pth")
    ...
    def auto_resume_training(self):
        # list all folders in output_directory/model_name/models
        files = os.listdir(os.path.join(self.log_path, "models"))
        # get all folders that contain numbers
        epoch_nums = [int(x.split('_')[1]) for x in files]
        # get latest folder
        last_epoch = max(epoch_nums) if epoch_nums else 0
        # checkpoint file
        checkpoint_folder = os.path.join(self.log_path, "models", 'weights_' + str(last_epoch))

        try:
            print(f"Loading checkpoint '{checkpoint_folder}'...")
            self.model.load_state_dict(torch.load(os.path.join(checkpoint_folder, 'segformer.pth')))
            return last_epoch
        except:
...
class IoU:
    ...
    def __init__(self, num_classes, dataset, ignore_index=None):
        ...
        self.classes = dict(woodscape_raw=["void", "road", "lanemarks", "curb", "person",
                            ...
                            kitti = ["void", "road", "lanemarks", "curb", "person", "rider", "vehicles", "bicycle", "motorcycle", "traffic_sign", "dummy11", "dummy12", "dummy13", "dummy14", "dummy15", "dummy16", "dummy17", "dummy18", "dummy19", "dummy20", "dummy21", "dummy22", "dummy23", "dummy24", "dummy25", "dummy26", "dummy27", "dummy28"],
